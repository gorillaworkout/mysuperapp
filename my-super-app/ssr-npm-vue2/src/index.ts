...
export function RouterView(VueInstance) {
  return VueInstance.component('router-view', {
    functional: true,
    render(h, { parent }) {
      let route = parent.$route;
      if (!route) {
...
export function RouterLink(VueInstance) {
  return VueInstance.component('router-link', {
    props: {
      to: { type: [String, Object], required: true },
      replace: { type: Boolean, default: false },
      activeClass: { type: String, default: 'active' },
