...
export default async function(rc: any) {
    const app = createApp({
        template: '<div class="container"><h1>E-Commerce App</h1><p>Welcome to the E-Commerce micro-app!</p></div>'
    });

    try {
        const html = '<div class="container"><h1>E-Commerce App</h1><p>Welcome to the E-Commerce micro-app!</p></div>';
        await rc.commit();
...
<body>
  ...
  ${rc.moduleEntry()}
        ...
        console.log('[E-Commerce SSR] Render complete');
    } catch (error) {
        console.error('[E-Commerce SSR] Render error:', error);
        rc.html = `<!DOCTYPE html>
<html>
<head><title>E-Commerce - Error</title></head>
<body>
