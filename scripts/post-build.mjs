...
function findEntryClient(appDir) {
  ...
  if (!existsSync(clientSrcDir)) {
    console.warn(`⚠️  ${appDir}: client/src directory not found at ${clientSrcDir}`);
    return null;
  }
  ...
  try {
    ...
    console.warn(`⚠️  ${appDir}: entry.client.*.final.mjs not found in ${clientSrcDir}`);
    return null;
  } catch (err) {
    console.error(`❌ ${appDir}: Error reading directory: ${err.message}`);
    return null;
  }
}
...
function RouterLink(VueCtor) {
  return VueCtor.component('router-link', {
    ...
    render(h) {
      const router = this.$router || (Router.getInstance && Router.getInstance());
      const path = typeof this.to === 'string' ? this.to : this.to.path;
      ...
      const data = {
        ...
        on: {
          click: (e) => {
            e.preventDefault();
            if (router) router.push(path, this.replace);
          }
        },
        attrs: { href: path }
      };
    ...
    app.component('router-link', {
      ...
      setup(props, { slots }) {
        ...
        const handleClick = (e) => {
          e.preventDefault();
          // Use sharedRouter for cross-app navigation (if available), fallback to localRouter
          const router = sharedRouter || localRouter;
          if (props.replace) router.replace(path.value);
          else router.push(path.value);
        };
...
`;
  ...
  try {
    ...
    console.log(`✅ ${appDir}: Created ${indexPath} (custom provider entry)`);
    return true;
  } catch (err) {
    console.error(`❌ ${appDir}: Failed to create provider index.mjs: ${err.message}`);
    return false;
  }
}
...
`;
  ...
  try {
    writeFileSync(indexPath, content, 'utf-8');
    console.log(`✅ ${appDir}: Created ${indexPath} → ./${file}`);
    return true;
  } catch (err) {
    console.error(`❌ ${appDir}: Failed to create index.mjs: ${err.message}`);
    return false;
  }
}
...
`;
  ...
  try {
    ...
    console.log(`✅ ${appDir}: Created ${rootIndexPath}`);
    return true;
  } catch (err) {
    console.error(`❌ ${appDir}: Failed to create root index.mjs: ${err.message}`);
    return false;
  }
}
...
`;
    ...
    console.log(`✅ ${appDir}: Created ${vueStablePath} → ./${vueFile}`);
    return true;
  } catch (err) {
    console.error(`❌ ${appDir}: Failed to create vue.mjs: ${err.message}`);
    return false;
  }
}
...
function createSharedModuleIndexes() {
  ...
  if (!existsSync(clientDir)) {
    console.warn('⚠️  ssr-npm-base: client directory not found');
    return false;
  }

  const esmxDir = join(clientDir, '@esmx');
  if (!existsSync(esmxDir)) {
    console.warn('⚠️  ssr-npm-base: @esmx directory not found');
    return false;
  }
...
`;
        writeFileSync(indexPath, content, 'utf-8');
        console.log(`✅ ssr-npm-base: Created ${indexPath} → ../${moduleFile}`);
      }
    }
  } catch (err) {
    console.error(`❌ ssr-npm-base: Failed to create shared module indexes: ${err.message}`);
    success = false;
  }
...
`;
  try {
    ...
    console.log(`✅ ssr-npm-base: Created ${rootIndexPath}`);
  } catch (err) {
    console.error(`❌ ssr-npm-base: Failed to create root index: ${err.message}`);
    success = false;
  }
 ...
 */
function main() {
  console.log('🔧 Post-build: Creating stable index.mjs entry points...\n');
  ...
  for (const app of microApps) {
    ...
    if (entryInfo) {
      ...
      if (clientSuccess || rootSuccess || vueSuccess) {
        successCount++;
      } else {
        failCount++;
      }
